...
if i==1
   ...
   phi2(i,1)=phi2(i-1,1)+0.01;
end
n(i,1)=1;
theta(i,1)=(2*Wa(i,1)*R1/m)*(((1+M1)/(M1*p*phi1))-1);
beta(i,1)=theta(i,1)+(2*Wa(i,1)*R1*M2*(1+M1)/(m*phi1))+2*Wa(i,1)*R1*(1-M2)/m;
...
TH(i,2)=max(TH(i,1)+phi2(i,1)-U1(i,1),0);
...
TH(i,t+1)=max(TH(i,t)+phi2(i,1)-U1(i,t),0);
end
end
    if d==1
        Ia1=Ia;
...
ylabel('Average UoI at Query'); 
legend({'Z=1','Z=8','Z=16','Z=64','Z=256',},'Location','northeast');
